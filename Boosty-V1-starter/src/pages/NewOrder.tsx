...
export default function NewOrder() {
  ...
  const filtered = useMemo(
    () => services.filter((s) => !cat || s.category_id === cat),
    [services, cat]
  );
  ...
  const charge = selected
    ? (qty / 1000) * selected.rate_per_1000
    : 0;
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (
      qty < selected.min_quantity ||
      qty > selected.max_quantity
    ) {
      setError(
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <p className="text-sm text-boosty-300">Order</p>
      ...
      <form
        onSubmit={submit}
        className="card mt-7 space-y-5 p-6"
      >
        <div>
        ...
        {selected && (
          <div className="rounded-xl border border-boosty-500/20 bg-boosty-500/5 p-4 text-sm text-slate-300">
            ...
            <div className="mt-3 flex flex-wrap gap-3 text-xs text-slate-400">
              ...
              <span>
                Max {selected.max_quantity.toLocaleString()}
              </span>
